...
class ShakespeareTask(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        ppl = torch.exp(loss)
        bpc = loss / math.log(2)
        self.log_dict({"val_loss": loss, "val_ppl": ppl, "val_bpc": bpc}, prog_bar=True)

    def test_step(self, batch, batch_idx):
        loss, logits, targets = self._shared_step(batch)
        ppl = torch.exp(loss)
        bpc = loss / math.log(2)
        self.log_dict({"test_loss": loss, "test_ppl": ppl, "test_bpc": bpc}, prog_bar=True)
...
class ShakespeareCharTask(ShakespeareTask):
    ...
    def test_step(self, batch, batch_idx):
        loss, logits, targets = self._shared_step(batch)
        bpc = loss.item() / math.log(2)
        self.log_dict({"test_loss": loss, "test_bpc": bpc}, prog_bar=True)


class ShakespeareTaskScheduler(LightningModule):
    def __init__(self, model, lr=1e-3):
        super().__init__()
        self.model = model
        self.lr = lr
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            ...
            "monitor": "val_loss",  # This must match what you log during validation
            "interval": "epoch",
            "frequency": 1
     }
...
class ShakespeareCharTaskScheduler(ShakespeareTaskScheduler):
    def validation_step(self, batch, batch_idx):
        loss, logits, targets = self._shared_step(batch)
        bpc = loss.item() / math.log(2)
        self.log_dict({"val_loss": loss, "val_bpc": bpc}, prog_bar=True)
